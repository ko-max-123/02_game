...
replaceText(41, "戦闘は専用部品、状態は限定GAS、内容はデータ駆動", "ランタイムは、ゲームプレイ・コンテンツ・基盤を分離する");
...
for (let index = 0; index < matches.length; index += 1) {
  ...
  const enemyLabels = enemyLines.map((line) => {
    const id = line.match(/([A-Z][A-Z0-9_]+)/)?.[1];
    return id ?? line;
  });
  ...
  for (let stage = 0; stage < 5; stage += 1) {
    ...
    setCell(slideNumber, stage + 1, 2, gimmicks[stage] ?? gimmicks[gimmicks.length - 1]);
  }
